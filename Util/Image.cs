...
namespace Util
{
    ...
    public static void ZoomAuto(Image initImage, string savePath, Double targetWidth, 
            Double targetHeight, string watermarkText, string watermarkImage)
        {
            //创建目录
        ...
        private static byte GetAvg(byte r, byte g, byte b)
        {
            return (byte)((r + g + b) / 3);
        }
        ...
        public static  Bitmap CreateBitmap(Image image)
        {
            ...
            Bitmap bitmap_temp = new Bitmap(image.Width, image.Height);
            Graphics g_temp = Graphics.FromImage(bitmap_temp);

            g.SmoothingMode = SmoothingMode.AntiAlias; //使绘图质量最高，即消除锯齿
            g.InterpolationMode = InterpolationMode.HighQualityBicubic;
            g.CompositingQuality = CompositingQuality.HighQuality;
            g_temp.DrawImage(image, new Rectangle(0, 0, image.Width, image.Height), 0, 0, bitmap.Width, bitmap.Height, GraphicsUnit.Pixel);
            TextureBrush _brush = new TextureBrush(bitmap_temp);


            g.FillEllipse(_brush, 0, 0, image.Width, image.Height);
            g.Save();
            g.Dispose();
            return bitmap;
            //Load(_path + Path.GetFileNameWithoutExtension(userHeadPath) + ".png");
        }
    }//end class
}
